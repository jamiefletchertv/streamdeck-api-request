...
function connected(jsn) {
    ...
    const actions = [
        ...
        'com.github.mjbnz.sd-api-request-cam'
    ];

    actions.forEach(actionUUID => {
        $SD.on(`${actionUUID}.didReceiveSettings`, jsonObj =>
            actionHandlers[actionUUID].onDidReceiveSettings(jsonObj)
        );
        $SD.on(`${actionUUID}.willAppear`, jsonObj =>
            actionHandlers[actionUUID].onWillAppear(jsonObj)
        );
        $SD.on(`${actionUUID}.willDisappear`, jsonObj =>
            actionHandlers[actionUUID].onWillDisappear(jsonObj)
        );
        $SD.on(`${actionUUID}.keyUp`, jsonObj =>
            actionHandlers[actionUUID].onKeyUp(jsonObj)
        );
    });
}
...
const actionHandlers = {
  //'com.github.mjbnz.sd-api-request': createActionHandler('APIRequest')
    'com.github.mjbnz.sd-api-request': createActionHandler('APIRequest'),
    'com.github.mjbnz.sd-api-request-cam': createActionHandler('CAMRequest')
}
...
function createActionHandler(type) {
    return {
        ...
        onDidReceiveSettings: function(jsn) {
            ...
            const request = this.cache[jsn.context];

            if (!settings || !request) return;

            request.updateSettings(settings);
            this.cache[jsn.context] = request;
        },
        ...
        onWillAppear: function(jsn) {
            ...
            const request = new window[type](jsn);
            this.cache[jsn.context] = request;
        },
        ...
        onWillDisappear: function(jsn) {
            ...
            let request = this.cache[jsn.context];
...
function CAMRequest(jsonObj) {
    ...
    function sendRequest(do_status_poll = false) {
        ...
        fetch(url, opts)
            ...
            .catch(err => { $SD.api.showAlert(context); log(err); }
        );
    ...
    async function checkResponseStatus(resp) {
        ...
        // Add Response Logging for Debug purposes
        //log('****** Response from ********', resp.url, resp);
